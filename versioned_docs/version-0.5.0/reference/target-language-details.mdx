...
The `time_value` variable will be of type `TimeValue`, which is an object used to represent a time in the TypeScript Target.
...
- **Miscellaneous**
  ...
  - Defining and recording tracepoints, such as `register_user_trace_event` and `tracepoint` (See [Tracing API](https://www.lf-lang.org/reactor-c/group__Tracing.html))
...
The Python target is built on top of the C runtime to enable maximum efficiency where possible. It uses the single-threaded C runtime by default but will switch to the multi-threaded C runtime if a physical action is detected.
