...
import UC_Microsteps from '../assets/code/uc/src/Microsteps.lf';
...
<NoSelectorTargetCodeBlock c={C_Microsteps} uc={UC_Microsteps} cpp={Cpp_Microsteps} py={Py_Microsteps} rs={Rs_Microsteps} ts={TS_Microsteps} lf />
...
<ShowOnly uc>
Notice the call to `env->scheduler->current_tag(env->scheduler)`.
This is a function that returns the current tag, which is a struct that contains the logical time and the microstep.
</ShowOnly>
...
import UC_Simultaneous from '../assets/code/uc/src/Simultaneous.lf';
...
<NoSelectorTargetCodeBlock c={C_Simultaneous} uc={UC_Simultaneous} cpp={Cpp_Simultaneous} py={Py_Simultaneous} rs={Rs_Simultaneous} ts={TS_Simultaneous} lf />
