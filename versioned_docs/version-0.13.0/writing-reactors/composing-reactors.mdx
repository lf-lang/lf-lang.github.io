...
import UC_RegressionTest from '../assets/code/uc/src/RegressionTest.lf';
...
<NoSelectorTargetCodeBlock c={C_RegressionTest} uc={UC_RegressionTest} cpp={Cpp_RegressionTest} py={Py_RegressionTest} rs={Rs_RegressionTest} ts={TS_RegressionTest} lf />
...
An import statement has one of the following forms:

```lf
  import classname as alias from "path"
  import classname as alias from <package/file>
```

where `classname as alias` can be a comma-separated list to import multiple reactors from the same file.
The `as alias` portion is optional and specifies alternative class names to use in the `new` statements.
In the first form, the `path` specifies another `.lf` file relative to the location of the current file.
For example:

```lf
import ReactorA, ReactorB as B from "path/to/Reactor.lf"
```

In the second form, the `<package/file>` specifies a [library file](../glossary/glossary.mdx#library-file) within an installed [package](../glossary/glossary.mdx#package), as described in [Importing Packages](importing-packages.mdx).
...
import UC_Hierarchy from '../assets/code/uc/src/Hierarchy.lf';
...
<NoSelectorTargetCodeBlock c={C_Hierarchy} uc={UC_Hierarchy} cpp={Cpp_Hierarchy} py={Py_Hierarchy} rs={Rs_Hierarchy} ts={TS_Hierarchy} lf />
