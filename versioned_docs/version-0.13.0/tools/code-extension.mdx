...
<Image img={diagram_icon} style={{maxWidth: 40}} />

By default, the diagram is rendered to the right of the editor.
This is usually inconvenient because the diagrams and the code both tend to be wide.
You can drag the diagram to the bottom, or, more usefully, change the default:

1. Go to Code>Settings>Settings (or Command-,)
2. Select the User tab to change your personal settings (or Workspace for just this workspace)
3. Search for "workbench.editor.openSideBySideDirection".
4. Change the setting from right to down.
