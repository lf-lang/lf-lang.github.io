...
Here, logical time is lagging physical time by a few milliseconds. Note that, unless the [fast option](../reference/target-declaration.mdx#fastfast) is given, logical time _t_ chases physical time _T_, so _t_ < _T_. Hence, the event being scheduled in the reaction to input `x` is assured of being in the future in logical time.
