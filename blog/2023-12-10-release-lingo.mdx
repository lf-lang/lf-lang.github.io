...
Programming languages like Python or Rust are popular not only because of particular language features, but also thanks to the quality of the ecosystem tools and packages available to programmers. 
A good and mature ecosystem includes features such as developer support, mainly through IDEs or LSPs, and the second important area concerns build tools and package management.
In the first area, the Lingua Franca already shines with its VSCode integration that offers functionality like code highlighting, error handling, build support, and diagram synthesis. Although it is possible to import reactors from files in the local file system, support for packaging in Lingua Franca is still in its infancy.
