...
// node packages/lingua-franca/scripts/getTypeScriptReleasePlan.js
...
const go = async () => {
  ...
  if (!upcoming) {
    return sendTeamsFail(
      `Could not find an iteration plan issue for ${versionMeta.tags.next} during the most recent site deploy - see https://github.com/microsoft/TypeScript-website/blob/v2/packages/lingua-franca/scripts/getTypeScriptReleaseInfo.js`
    )
  }
  ...
  if (missing.length) {
    // prettier-ignore
    return sendTeamsFail(`Could not parse the md table for ${missing.join(",")} in https://github.com/microsoft/TypeScript/issues/${upcoming.number} - see https://github.com/microsoft/TypeScript-website/blob/v2/packages/lingua-franca/scripts/getTypeScriptReleaseInfo.js`)
  }
