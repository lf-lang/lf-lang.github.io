...
The `startup` trigger causes the reaction to execute at the logical start time of the program. The body of the reaction, delimitted by `{= ... =}`, is ordinary code in the target language. Each target language provides mechanisms for accessing inputs, parameters, and state variables within the reaction body, as explained below.

## Structure of an LF Project

The Lingua Franca tools assume that every `.lf` program is put within a directory called `src`. The parent of the `scr` directory is defined to be the **project root**. If you create file `Minimal.lf` with the above content and put it into a `src` directory, then you can compile it using the `lfc` command-line compiler or your favorite IDE. This will create two more directories within the project root, `src-gen` and `bin`. The `src-gen` directory will have one or more generated files in the target language.

If the target language is a compiled language (like C, C++, and Rust), then, after compiling `Minimal.lf`, the `bin` directory will have an executable file called `Minimal`. Executing that file will result in output that looks something like this (the exact form of the output varies by target):
...
If the target language is an interpreted language (like Python and TypeScript), then you run the program using the generated file in `src-gen`.
