...
&nbsp;
<label for="cpp">
 <input type="checkbox" id="cpp" name="target_language" value="cpp" checked="false" onclick="var checked = this.checked ? 'block' : 'none';
    var elements = document.getElementsByClassName('Cpp');
    for (var i = 0; i < elements.length; i++) {
        elements[i].style.display = checked;
    }">C++
</label>
...
&nbsp;
<label for="python">
 <input type="checkbox" id="python" name="target_language" value="python" checked="false" onclick="var checked = this.checked ? 'block' : 'none';
...
One useful type predefined by LF is the `time` type, which represents time durations. Values of this type may be written with *time expressions*, like `100 msec` or `1 second` (see [Basic expressions](#basic-expressions) for a reference).
