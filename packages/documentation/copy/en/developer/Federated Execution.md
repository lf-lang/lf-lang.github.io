...
In code generator, each federate attempts to connect with an RTI at the `IP address`. For example, it opens a TCP connection. The number of `STARTING_PORT + PORT_RANGE_LIMIT` will be the maximum number if it is successfully connected.
...
- `MSG_TYPE_REJECT`: The RTI will respond with this message if the federation IDs do not match and close the connection. At this point the federate will increment the port number and try again to find an RTI that matches.
...
- **Initial clock synchronization**
  ...
  - `LF_CLOCK_SYNC_EXCHANGES_PER_INTERVAL` **times** of cycle will happen to account for network delay. The cycle is like as below:
    ...
    - `MSG_TYPE_CLOCK_SYNC_T3`: A reply message to RTI after "T2". It records the time (T3) at which this message has gone out. `#QUESTION: Does "it" means federate?`
      - Payload: Federate ID
...
#### Estimating **the average clock synchronization error `E`**
...
#### **Case 1**) **Decentralized** coordination parameter and **inbound connections** of federate #QUESTION: What does this means?
...
### 6. Tag message depends on the coordination type:

#### **Case 1**) **Centralized** coordination

- `MSG_TYPE_NEXT_EVENT_TAG`: Each federate sends this message to the RTI with the start tag. So, each federate can say that they have the valid event at the **start tag (start time, 0)** and inform the RTI of this event.
- `MSG_TYPE_LOGICAL_TAG_COMPLETE`: Each federate sends this message at the conclusion of each tag.
- `MSG_TYPE_TAG_ADVANCE_GRANT` and `MSG_TYPE_PROVISIONAL_TAG_ADVANCE_GRANT`: Each federate would have to wait for one of these messages before it can advance to a particular tag.
...
/\*_ Byte identifying that the federate is ending its execution. _/
...
/\*_ Federation ID does not match. _/
#define FEDERATION_ID_DOES_NOT_MATCH 1

/\*_ Federate with the specified ID has already joined. _/
#define FEDERATE_ID_IN_USE 2

/\*_ Federate ID out of range. _/
#define FEDERATE_ID_OUT_OF_RANGE 3

/\*_ Incoming message is not expected. _/
#define UNEXPECTED_MESSAGE 4

/\*_ Connected to the wrong server. _/
#define WRONG_SERVER 5

#endif /_ NET_COMMON_H _/
