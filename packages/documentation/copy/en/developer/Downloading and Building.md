...
In fact, running `./bin/lfc-dev <args>` is equivalent to running `./gradlew cli:lfc:run --args="<args>"`.
...
Thus, you can choose if you want to use `bin/lfc-dev`, which runs lfc through gradle and always uses the latest code, or if you prefer to run `./gradlew build` and then directly invoke `build/install/lf-cli/bin/lfc`.
