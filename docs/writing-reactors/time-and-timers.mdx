...
The **lag** is allowed to go negative only if the [`fast`](../reference/target-declaration.mdx#fast) target property, the [`--fast`](../reference/target-declaration.mdx#command-line-arguments) command-line argument, or the `@fast(true)` attribute is set to `true` on the main reactor. In that case, the program will execute as fast as possible with no regard to physical time.

<ShowOnly c uc cpp rs>
...
import UC_SlowingClock from '../assets/code/uc/src/SlowingClock.lf';
...
<NoSelectorTargetCodeBlock c={C_SlowingClock} uc={UC_SlowingClock} cpp={Cpp_SlowingClock} py={Py_SlowingClock} rs={Rs_SlowingClock} ts={TS_SlowingClock} lf />
...
import UC_Timer from '../assets/code/uc/src/Timer.lf';
...
<NoSelectorTargetCodeBlock c={C_Timer} uc={UC_Timer} cpp={Cpp_Timer} py={Py_Timer} rs={Rs_Timer} ts={TS_Timer} lf />
...
<ShowOnly uc inline>`env->get_logical_time(env)`</ShowOnly>
...
import UC_TimeElapsed from '../assets/code/uc/src/TimeElapsed.lf';
...
<NoSelectorTargetCodeBlock c={C_TimeElapsed} uc={UC_TimeElapsed} cpp={Cpp_TimeElapsed} py={Py_TimeElapsed} rs={Rs_TimeElapsed} ts={TS_TimeElapsed} lf />
...
The starting logical time is obtained from local physical clock on the execution platform.
For many platforms, this will be an epoch time, which gives the number of elapsed
nanoseconds since midnight on January 1, 1970.
<ShowOnly c inline>This starting time can be obtained by calling `lf_time_start()`.</ShowOnly>
<ShowOnly py inline>This starting time can be obtained by calling `lf.time.start()`.</ShowOnly>
<ShowOnly uc inline>This starting time is available in reactions as `env->scheduler->start_time`.</ShowOnly>

<ShowOnly c>
In the `C` target,
the start time can be postponed to be a multiple of a specified time by giving a command-line
argument:

```
bin/MyProgram --start-time-multiple <num> <units>
```

or

```
bin/MyProgram -m <num> <units>
```

where `<num>` is a positive
integer and `<units>` is one of the above units.
This can make logical times more readable.
For example, with the above Timer example, we get:

```
% bin/Timer -m 1 s
---- Start execution on Wed Jun 10 10:19:16 2026 ---- plus 0 nanoseconds
Logical time is 1781111956000000000.
Logical time is 1781111957000000000.
Logical time is 1781111958000000000.
...
```

The start time is a multiple of one second.
</ShowOnly>
...
import UC_TimeLag from '../assets/code/uc/src/TimeLag.lf';
...
<NoSelectorTargetCodeBlock c={C_TimeLag} uc={UC_TimeLag} cpp={Cpp_TimeLag} py={Py_TimeLag} rs={Rs_TimeLag} ts={TS_TimeLag} lf />
...
A reaction is always invoked at a well-defined logical time, and logical time does not advance during its execution. Any output produced by the reaction will be **logically simultaneous** with the input. In other words, reactions are **logically instantaneous** (for an exception, see [Generalizing Logical Execution Time](https://doi.org/10.1007/978-3-031-22337-2_8)). Physical time, however, does elapse during execution of a reaction.

## Timeout

By default, a Lingua Franca program will terminate when there are no more events to process. If there is a timer with a non-zero period, then there will always be more events to process, so the default execution will be unbounded. To specify a finite execution horizon, you can either specify a [`timeout` target property](../reference/target-declaration.mdx#timeout), a [`--timeout` command-line option](../reference/target-declaration.mdx#command-line-arguments), or (for the uC target) the `@timeout(N)` attribute on the main reactor. For example, the following `timeout` property will cause the above timer with a period of one second to terminate after 11 events:
...
import UC_TestCount from '../assets/code/uc/src/TestCount.lf';
...
<NoSelectorTargetCodeBlock c={C_TestCount} uc={UC_TestCount} cpp={Cpp_TestCount} py={Py_TestCount} rs={Rs_TestCount} ts={TS_TestCount} lf />
