...
import UC_Double from '../assets/code/uc/src/Double.lf';
...
<NoSelectorTargetCodeBlock c={C_Double} uc={UC_Double} cpp={Cpp_Double} py={Py_Double} rs={Rs_Double} ts={TS_Double} lf />
...
import UC_Destination from '../assets/code/uc/src/Destination.lf';
...
<NoSelectorTargetCodeBlock c={C_Destination} uc={UC_Destination} cpp={Cpp_Destination} py={Py_Destination} rs={Rs_Destination} ts={TS_Destination} lf />
...
<ShowIf uc>
Unless you pass pointer types from one reactor to another, an `input->value` may be freely modified by a destination reactor.
Micro-LF copies values when there are multiple destinations, so this will not lead to nondeterministic behavior.
</ShowIf>
</ShowIfs>