...
The preservation of timestamps across federates implies some constraints (unless you use [physical connections](#physical-connections)). <ShowOnly c py inline>How these constraints are managed depends on whether you choose **centralized** or **decentralized** coordination.</ShowOnly><ShowOnly ts inline>For the TypeScript target, only **centralized** coordination is currently supported.</ShowOnly>
...
<ShowOnly c py inline>For the C and Python targets, Lingua Franca provides end-to-end communication security that encrypts all message exchanges and ensures only authorized federates can participate using pluggable secure communication backends like TLS or SST.
