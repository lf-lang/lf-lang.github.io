...
`"defer"` (the default), `"drop"`, `"replace"`, or `"update"`. Note that the quotation marks are needed.

<ShowOnly uc>
In micro-LF, an action needs an attribute to specify how much memory to allocate for storing
scheduled events. This is done as follows:
...
The `<number>` is an integer literal that specifies the maximum number of unprocessed
events that can be on the event queue for the action.

If no `@max_pending_events` attribute is given, then the default is 1.
If and when the maximum number of pending events is reached, new events will be handled
according to the `<policy>` argument, as described in the next section, except that
the "defer" policy will be handled by dropping the event.

</ShowOnly>

<ShowOnly c uc cpp ts rs>
...
import UC_Schedule from '../assets/code/uc/src/Schedule.lf';
...
<NoSelectorTargetCodeBlock c={C_Schedule} uc={UC_Schedule} cpp={Cpp_Schedule} py={Py_Schedule} rs={Rs_Schedule} ts={TS_Schedule} lf />
...
<ShowIfs>
  <ShowIf c uc>
    ...
    The arguments to the `lf_schedule()` function are the action named `a` and a time. The action `a` has to be declared as an effect of the reaction in order to reference it in the call to `lf_schedule()`. If you fail to declare it as an effect (after the `->` in the reaction signature), then you will get a compilation error.
...
import UC_Physical from '../assets/code/uc/src/Physical.lf';
...
<NoSelectorTargetCodeBlock c={C_Physical} uc={UC_Physical} cpp={Cpp_Physical} py={Py_Physical} rs={Rs_Physical} ts={TS_Physical} lf />
...
import UC_Asynchronous from '../assets/code/uc/src/Asynchronous.lf';
...
<NoSelectorTargetCodeBlock c={C_Asynchronous} uc={UC_Asynchronous} cpp={Cpp_Asynchronous} py={Py_Asynchronous} rs={Rs_Asynchronous} ts={TS_Asynchronous} lf />
...
- **"replace" and "update"** govern how a new event interacts with an _existing pending_ event still on the event queue. They modify or remove a pending event rather than simply accepting or rejecting the new one.

**"replace" vs. "update".** The key difference is which event's time is kept:
- `"replace"` keeps the _earlier_ event's time and updates its payload with the new event's payload.
...
### Watchdog Timer Pattern Using "update"

The `"update"` policy is useful for implementing a logical-time-aware watchdog timer. For example, in a Raft consensus protocol, an election timeout can be modeled as follows:
...
This pattern works correctly for both centralized and decentralized coordination because it operates in logical time.
...
Just like for inputs, this can be done in the  <ShowOnly c inline>C target with `a->is_present`</ShowOnly>
  <ShowOnly uc inline>uC target with `lf_is_present(a)`</ShowOnly>
