...
Reactors can contain instances of other reactors defined in the same file or in an imported file. Assume the `Count` and `Scale` reactors defined in [Parameters and State Variables](<../topics/Parameters and State Variables.mdx>) are stored in files `Count.lf` and `Scale.lf`, respectively,
and that the `TestCount` reactor from [Time and Timers](<../topics/Time and Timers.mdx>) is stored in `TestCount.lf`. Then the following program composes one instance of each of the three:

import C_RegressionTest from '../code/c/src/RegressionTest.lf';
import Cpp_RegressionTest from '../code/cpp/src/RegressionTest.lf';
import Py_RegressionTest from '../code/py/src/RegressionTest.lf';
import Rs_RegressionTest from '../code/rs/src/RegressionTest.lf';
import TS_RegressionTest from '../code/ts/src/RegressionTest.lf';
...
A bank with several instances can be created in one such statement, as explained in the [banks of reactors documentation](<../topics/Multiports and Banks.mdx#banks-of-reactors>).
...
import C_Hierarchy from '../code/c/src/Hierarchy.lf';
import Cpp_Hierarchy from '../code/cpp/src/Hierarchy.lf';
import Py_Hierarchy from '../code/py/src/Hierarchy.lf';
import Rs_Hierarchy from '../code/rs/src/Hierarchy.lf';
import TS_Hierarchy from '../code/ts/src/Hierarchy.lf';
...
The $after$ keyword specifies that the logical time of the event delivered to the destination port will be larger than the logical time of the reaction that wrote to source port. The time value is required to be non-negative, but it can be zero, in which case the input event at the receiving end will be one [microstep](<../topics/Actions.mdx#superdense-time>) later than the event that triggered it.
...
In such a connection, the logical time at the recipient is derived from the local physical clock rather than being equal to the logical time at the sender. The physical time will always exceed the logical time of the sender (unless fast is set to `true`), so this type of connection incurs a nondeterministic positive logical time delay. Physical connections are useful sometimes in [Distributed-Execution](<../topics/Distributed Execution.mdx>) in situations where the nondeterministic logical delay is tolerable. Such connections are more efficient because timestamps need not be transmitted and messages do not need to flow through through a centralized coordinator (if a centralized coordinator is being used).
