...
The **lag** is allowed to go negative only if the [`fast` target property](<../reference/Target Declaration.mdx#fast>) or the [`--fast`](<../reference/Target Declaration.mdx#command-line-arguments>) command-line argument is set to `true`. In that case, the program will execute as fast as possible with no regard to physical time.
...
import C_SlowingClock from '../code/c/src/SlowingClock.lf';
import Cpp_SlowingClock from '../code/cpp/src/SlowingClock.lf';
import Py_SlowingClock from '../code/py/src/SlowingClock.lf';
import Rs_SlowingClock from '../code/rs/src/SlowingClock.lf';
import TS_SlowingClock from '../code/ts/src/SlowingClock.lf';
...
import C_Timer from '../code/c/src/Timer.lf';
import Cpp_Timer from '../code/cpp/src/Timer.lf';
import Py_Timer from '../code/py/src/Timer.lf';
import Rs_Timer from '../code/rs/src/Timer.lf';
import TS_Timer from '../code/ts/src/Timer.lf';
...
This target provides a built-in function for retrieving the logical time at which the reaction is invoked, <ShowOnly c>`get_logical_time()`</ShowOnly>
...
import C_TimeElapsed from '../code/c/src/TimeElapsed.lf';
import Cpp_TimeElapsed from '../code/cpp/src/TimeElapsed.lf';
import Py_TimeElapsed from '../code/py/src/TimeElapsed.lf';
import Rs_TimeElapsed from '../code/rs/src/TimeElapsed.lf';
import TS_TimeElapsed from '../code/ts/src/TimeElapsed.lf';
...
import C_TimeLag from '../code/c/src/TimeLag.lf';
import Cpp_TimeLag from '../code/cpp/src/TimeLag.lf';
import Py_TimeLag from '../code/py/src/TimeLag.lf';
import Rs_TimeLag from '../code/rs/src/TimeLag.lf';
import TS_TimeLag from '../code/ts/src/TimeLag.lf';
...
A reaction is always invoked at a well-defined logical time, and logical time does not advance during its execution. Any output produced by the reaction will be **logically simultaneous** with the input. In other words, reactions are **logically instantaneous** (for an exception, see [Logical Execution Time](<../less-developed/Logical Execution Time.mdx>)). Physical time, however, does elapse during execution of a reaction.

## Timeout

By default, a Lingua Franca program will terminate when there are no more events to process. If there is a timer with a non-zero period, then there will always be more events to process, so the default execution will be unbounded. To specify a finite execution horizon, you can either specify a [`timeout` target property](<../reference/Target Declaration.mdx#timeout) or a [`--timeout` command-line option](docs/handbook/target-declaration#command-line-arguments>). For example, the following `timeout` property will cause the above timer with a period of one second to terminate after 11 events:
...
import C_TestCount from '../code/c/src/TestCount.lf';
import Cpp_TestCount from '../code/cpp/src/TestCount.lf';
import Py_TestCount from '../code/py/src/TestCount.lf';
import Rs_TestCount from '../code/rs/src/TestCount.lf';
import TS_TestCount from '../code/ts/src/TestCount.lf';
...
The **shutdown** trigger typically occurs at [microstep](<../topics/Actions.mdx#superdense-time>) 0, but may occur at a larger microstep. See [Superdense Time](<../topics/Actions.mdx#superdense-time>) and [Termination](<../topics/Termination.mdx>).
