...
import C_Double from '../code/c/src/Double.lf';
import Cpp_Double from '../code/cpp/src/Double.lf';
import Py_Double from '../code/py/src/Double.lf';
import Rs_Double from '../code/rs/src/Double.lf';
import TS_Double from '../code/ts/src/Double.lf';
...
The $reaction$ declaration above indicates that an input event on port `x` is a **trigger** and that an output event on port `y` is a (potential) **effect**. A reaction can declare more than one trigger or effect by just listing them separated by commas (See [Reactions](<../topics/Reactions.mdx>) for details). For example, the following reactor has two triggers and tests each input for presence before using it:

import C_Destination from '../code/c/src/Destination.lf';
import Cpp_Destination from '../code/cpp/src/Destination.lf';
import Py_Destination from '../code/py/src/Destination.lf';
import Rs_Destination from '../code/rs/src/Destination.lf';
import TS_Destination from '../code/ts/src/Destination.lf';
...
If one or more reactions set an output multiple times at the same [tag](<../topics/Superdense Time.mdx#tag-vs-time>), then only the last value set will be seen by any downstream reactors.
