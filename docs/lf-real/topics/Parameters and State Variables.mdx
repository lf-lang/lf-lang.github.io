...
Depending on the target, the type may be a [generic type](<../topics/Generics.mdx>), which means that the type is parameter determined at the time the reactor class is instantiated.
...
For example, the `Double` reactor on the [previous page](<../topics/Inputs and Outputs.mdx>) can be replaced with a more general parameterized reactor `Scale` as follows:

import C_Scale from '../code/c/src/Scale.lf';
import Cpp_Scale from '../code/cpp/src/Scale.lf';
import Py_Scale from '../code/py/src/Scale.lf';
import Rs_Scale from '../code/rs/src/Scale.lf';
import TS_Scale from '../code/ts/src/Scale.lf';
...
import C_Count from '../code/c/src/Count.lf';
import Cpp_Count from '../code/cpp/src/Count.lf';
import Py_Count from '../code/py/src/Count.lf';
import Rs_Count from '../code/rs/src/Count.lf';
import TS_Count from '../code/ts/src/Count.lf';
...
<ShowIfs>
  <ShowIf cpp ts rs>
    The $reset$ keyword is not supported in $target-language$ because [modal reactors](<../topics/Modal Models.mdx>) are not supported.
  </ShowIf>
  ...
  <ShowIf c py>
    ...
    />
    When this is done, if the state variable or the reactor is within a mode of a [modal reactor](<../topics/Modal Models.mdx>), then when the mode is entered via a reset transition, the state variable will be reset to its initial value. For details, see the [Modal Reactors](<../topics/Modal Models.mdx>) section.
