...
Tracing is different from [logging](target-declaration.mdx#logging). Logging produces human-readable output in textual form and incurs significant overhead. Tracing produces binary data that must be further processed to be useful and is designed to have minimal impact on the execution time of a program.
...
reactor Foo {
  # Handle for registered user trace event.
  state handle = 0

  reaction(startup) {=
    self.handle = self.register_user_trace_event("Description of event")
    if self.handle == 0:
...
The reaction to `startup` is designed to fail (`exit(1)`) if the `tracing` target parameter is not set to to `true`. If you wish to just ignore tracepoints if the target parameter is not set, then omit the `exit(1)` statement.
  </ShowIf>
</ShowIfs>
...
```
  ...
  <ShowIf py>
To then actually record an event, in a reaction, call `tracepoint_user_event`, passing it the same handle returned by `register_user_trace_event`. E.g.,
...
```lf-py
	reaction(in) -> out {=
	    ...
	    self.tracepoint_user_value(self.handle, 42);
	=}
```
  </ShowIf>
</ShowIfs>
...
In addition to the default file-based tracing described above, you can use the [lf-trace-xronos](https://github.com/lf-lang/lf-trace-xronos) plugin to stream live traces to the Xronos Dashboard using OpenTelemetry. This plugin provides real-time visualization of trace data without the need for post-processing.

To use the plugin, you need to build and install it, then configure your LF program to use it via the [`trace-plugin`](../reference/target-declaration.mdx#trace-plugin) target property. Refer to the [lf-trace-xronos repository](https://github.com/lf-lang/lf-trace-xronos) for detailed build instructions and usage examples.

![LF Trace via Xronos Dashboard](./../assets/images/tracing/LFTraceXronos.png)
