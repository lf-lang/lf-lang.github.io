...
For options to the target specification, see [detailed documentation of the target options](<../reference/Target Declaration.mdx>).
...
The supported target parameters and command-line options are documented in the [Target Declaration](<../reference/Target Declaration.mdx>) documentation.
...
- `LF_PRINT_DEBUG(format, ...)`: Use this for verbose messages that are only needed during debugging. Nothing is printed unless the [target](<../reference/Target Declaration.mdx#logging>) parameter `logging` is set to `debug`. THe overhead is minimized when nothing is to be printed.

- `LF_PRINT_LOG(format, ...)`: Use this for messages that are useful logs of the execution. Nothing is printed unless the [target parameter `logging`](<../reference/Target Declaration.mdx#logging>) is set to `log` or `debug`. This is a macro so that overhead is minimized when nothing is to be printed.

- `lf_print(format, ...)`: Use this for messages that should normally be printed but may need to be redirected to a user interface such as a window or terminal (see `register_print_function` below). These messages can be suppressed by setting the [logging target property](<../reference/Target Declaration.mdx#logging>) to `warn` or `error`.

- `lf_print_warning(format, ...)`: Use this for warning messages. These messages can be suppressed by setting the [logging target property](<../reference/Target Declaration.mdx#logging>) to `error`.

- `lf_print_error(format, ...)`: Use this for error messages. These messages are not suppressed by any [logging target property](<../reference/Target Declaration.mdx#logging>).
...
The Python supports the [logging](<../reference/Target Declaration.mdx#logging>) target specification. This will cause the runtime to produce more or less information about the execution. However, user-facing functions for different logging levels are not yet implemented (see issue [#619](https://github.com/lf-lang/lingua-franca/issues/619)).
...
Logging can also be turned on with the `--log-level` CLI option, if the application features a [CLI](<../reference/Target Declaration.mdx#command-line-arguments>).
...
By default, the C runtime system uses multiple worker threads in order to take advantage of multicore execution. The number of worker threads will match the number of cores on the machine unless the `workers` argument is given in the [target](<../reference/Target Declaration.mdx#threading>) statement or the `--workers` [command-line argument](<../reference/Target Declaration.mdx#command-line-arguments>) is given.
...
By giving the `single-threaded` [target option](<../reference/Target Declaration.mdx#single-threaded>) or the `--single-threaded` [command-line argument](<../reference/Target Declaration.mdx#command-line-arguments>), the generated program will execute the program using only a single thread. This option is most useful for creating programs to run on bare-metal microprocessors that have no threading support. On such platforms, mutual exclusion is typically realized by disabling interrupts.
...
2. At each logical time, pull all events from event queue that have the same earliest tag, find the reactions that these events trigger, and put them on the reaction queue. If there are no events on the event queue, then exit the program (unless the `--keepalive true` [command-line argument](<../reference/Target Declaration.mdx#command-line-arguments>) is given).

3. Wait until physical time matches or exceeds that earliest timestamp (unless the `--fast true` [command-line argument](<../reference/Target Declaration.mdx#command-line-arguments>) is given). Then advance logical time to match that earliest timestamp.
...
The Python target is built on top of the C runtime to enable maximum efficiency where possible. It uses the single-threaded C runtime by default but will switch to the multi-threaded C runtime if a physical action is detected. The [threading](<../reference/Target Declaration.mdx#threading>) target property can be used to override this behavior.
...
See [Target Declaration](<../reference/Target Declaration.mdx>) for the full list of supported target properties.

### The Executable

The executable name is the name of the main reactor _transformed to snake_case_: `main reactor RustProgram` will generate `rust_program`. See [Command-Line Arguments](<../reference/Target Declaration.mdx#command-line-arguments>) for details.
